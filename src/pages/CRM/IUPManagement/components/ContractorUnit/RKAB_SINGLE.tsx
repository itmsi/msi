...
import { useIupContactorUnit } from '../../hooks/useIupContactorUnit';
        ...
        <div className="w-full rounded-2xl border border-slate-300 bg-white">
            ...
            {(!dataRkab || dataRkab.length === 0) ? (
                ...
                <div className="divide-slate-300 divide-y">
                    {dataRkab.map((rkab, index) => (
                        <IupRkabCard
                            key={rkab.iup_rkab_id ?? `new-${index}`}
                            rkab={rkab}
                            index={index}
                            errors={rkabErrors[index] ?? {}}
                            isSubmitting={isSubmitting}
                            onChange={(field, value) => handleRkabFieldChange(index, field, value)}
                            onSave={() => handleSubmitRkab(rkab, index)}
                            onRemove={() => removeRkab(rkab, index)}
                        />
                    ))}
                </div>
            )}
