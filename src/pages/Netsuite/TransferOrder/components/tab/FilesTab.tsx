...
import { AttachFileItem } from '../../types/transferOrder';
...
const FilesTab: React.FC<FilesTabProps> = ({
        ...
        try {
            ...
            if (editingFile) {
                ...
                if (res.success) {
                    ...
                    toast.error((res as any).message || 'Gagal mengupdate file');
                }
                setEditingFile(null);
            } else {
                if (!selectedFile) {
    ...
    const handleDelete = async (row: AttachFileItemLocal) => {
        ...
        try {
            const activeToId = String(toId ?? id ?? '');
            const res = row.id
                ? await TransferOrderService.attachFileDeleteDetailTO(String(row.id), activeToId)
                : { success: true };
            if (res.success) {
                if (row.isNew) {
                ...
                toast.error((res as any).message || 'Gagal menghapus file');
            }
        } catch {
