...
// import { AttachFileItem } from '../../types/purchaseorder';
...
import { AttachFileItem } from '../../types/salesOrder';
...
const emptyEntry: AttachFileItem = { id: '', fileUrl: '', fileName: '' };
...
const FilesTab: React.FC<FilesTabProps> = ({
    ...
    // Simpan fileList asli dari server — tidak terpengaruh oleh onAddFiles accumulation
    ...
    // Gabungkan file dari server asli (minus yang dihapus + minus URL yang ada di newFiles) + newFiles
    const allFiles: AttachFileItemLocal[] = useMemo(() => {
        const newFileUrls = new Set(newFiles.map(f => f.fileUrl));
        const fromServer = serverFileList.current
            .filter(f => !deletedUrls.has(f.fileUrl) && !newFileUrls.has(f.fileUrl))
            .map(f => ({ ...f, isNew: false }));
        const fromNewFiles = newFiles.map(f => ({ ...f, isNew: !f.isEdited }));
        return [...fromServer, ...fromNewFiles];
    }, [newFiles, deletedUrls]);

    // Helper: rebuild dari server asli (minus deleted) + newFiles terkini → kirim ke parent
    const notifyParent = (updatedNewFiles: AttachFileItem[], updatedDeletedUrls: Set<string>) => {
        const existingFiles = serverFileList.current.filter(f => !updatedDeletedUrls.has(f.fileUrl));
    ...
    const handleAddEntry = async () => {
        ...
        // Saat mode add, file wajib dipilih. Saat edit, file boleh kosong (hanya rename)
        ...
        try {
            if (soId && editingFile) {
                ...
                const res = await SalesOrderService.attachFileSOUpdate({
                ...
                if (res.success) {
                    ...
                    if (editingFile.isNew || editingFile.isEdited) {
                        // Edit file yang sudah ada di newFiles (isNew atau isEdited) — update in-place
                        const updated = newFiles.map(item =>
                            item.fileUrl === editingFile.fileUrl && item.fileName === editingFile.fileName
                                ? { ...updatedEntry, isNew: editingFile.isNew, isEdited: editingFile.isEdited }
                                : item
                        );
                        setNewFiles(updated);
                ...
                const res = await SalesOrderService.attachFileSO({
                    file: selectedFile!,
                    file_name: entry.fileName,
    ...
    const handleEdit = (row: AttachFileItemLocal) => {
        setEditingFile(row);
        setEntry({ id: row.id ?? '', fileUrl: row.fileUrl, fileName: row.fileName });
        setShowForm(true);
        setEntryError('');
    };
    ...
    const handleDelete = async (row: AttachFileItemLocal) => {
        ...
        try {
            const res = await SalesOrderService.attachFileSODelete({ fileUrl: row.fileUrl });
            if (res.success) {
                if (row.isNew) {
