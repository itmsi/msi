...
import { formatDecimal } from './interviewFormHelpers';
...
const generateManualTable = (doc: any, formData: FormData, yPosition: number) => {
    ...
    const candidateInfo = [
        ...
        ['Age', candidateData.age_candidate || 'N/A', 'Duration', candidateData.duration_candidate ? `${candidateData.duration_candidate} m` : 'N/A'],
    ];
...
const getMultipliedScore = (companyValue: string, score: number | string) => {
    const numericScore = Number(score);

    switch (companyValue) {
        case 'SIAH':
            return formatDecimal(numericScore * 2);

        case '7 Values':
            return formatDecimal(numericScore * 1.7);

        case 'CSE':
            return formatDecimal(numericScore * 2);

        default:
            return formatDecimal(numericScore);
    }
};
...
const generateManualAssessmentTable = (doc: any, formData: FormData, yPosition: number) => {
    ...
    // Each formData.interview entry is one category submission wrapping a
    // detail_interviews array — flatten to individual {aspect, question, answer,
    // score} rows here, since that's the shape the row-rendering below expects.
    const groupedData: Record<string, any[]> = {};
    if (formData.interview && Array.isArray(formData.interview)) {
        formData.interview.forEach((item) => {
            const details = item.detail_interviews || [];
            details.forEach((detail) => {
                if (!groupedData[item.company_value]) groupedData[item.company_value] = [];
                groupedData[item.company_value].push(detail);
            });
        });
    }
    ...
    sortedCompanyValues.forEach((companyValue, companyIndex) => {
        ...
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(10);
        doc.text('TOTALsss', posisi_x + colCompanyWidth + colStandardWidth + 2, currentY + 5);
        ...
        totalWithMultiplier = formatDecimal(totalWithMultiplier);
